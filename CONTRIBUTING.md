...
Since many plugins need to connect to a Caddy instance to verify functionality, `pytest` will automatically start a Caddy server container that you can connect to.
You can configure your target to use this Caddy server like so:
...
dependencies:
  - setup_host
```
Check out the [`integration_config.yml` template](./tests/integration/integration_config.yml.j2) for all available variables.
