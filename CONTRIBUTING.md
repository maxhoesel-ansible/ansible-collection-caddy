...
The plugins in this collection provide access to Caddys API.
...
- Caddy version: The most recent stable version in the caddy repositories. In the future, we may also support distribution-provided versions
...
```bash
# Will run the py3-ansible7-roles-caddy_server environment
tox -e py3-ansible7-roles-caddy_server

# Will run XXX-roles-caddy_server environment with ansible versions 6,7 and 8
# Note the single quotes around the string!
tox -e 'py3-ansible{6,7,8}-roles-caddy_server'
```
...
Since many plugins in this collection need access to a caddy server, the `tox` environments spin up additional docker containers as needed -
...
For integration tests, you can get started by either copying an existing target such as [`caddy_config`](./tests/integration/targets/caddy_config/), or starting out from scratch.
Each target is an ansible role containing tasks that verify the functionality of a plugin.

To access the testing caddy servers API, you can use the values in the [integration_config.yml template](./tests/integration/integration_config.yml.template):
...
- name: Try to load basic configuration
  maxhoesel.caddy.caddy_load:
    config: "something"
    caddy_host: "{{ caddy_host }}" # <-- automatically loaded from the integration_config.yml
